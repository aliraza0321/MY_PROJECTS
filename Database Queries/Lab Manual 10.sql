...
if exists(
select * 
from  deleted  as d join orders o on d.c_id = o.c_id
)
begin
print('dont allow to delete this ')
end
else
begin
...
if exists (
select *
from inserted as  i join inventory as inv 
        on i.model = inv.modelno 
        and i.make = inv.make 
        and i.company = inv.company
)
begin
...
begin
print 'Not enough stock - update cancelled'
rollback
end
end
...
if exists (
select *  from deleted
where articles_available > 100
)
begin
print 'Cannot delete item with quantity > 100'
rollback
end
end
...
as
begin
insert into orders (c_id, d_id, make, company, model, dateorder, status_order, date_completeion)
select c_id,d_id,make,company,model,GETDATE(),   -- always current date
        status_order,
        date_completeion
from  inserted
end
...
values (@cname, @c_id, @c_address, @c_cnic, @contact)
set @flag = 0
end
END
...
exec SignupCustomer 
    'c999','ali','Lahore','12345-1234567-1','123456',@result1 
