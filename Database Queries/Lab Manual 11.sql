...
--Task2
BEGIN TRANSACTION;
DECLARE @oldValue DECIMAL(10,4);
-- Get current factor value
SELECT @oldValue = FactorValue
...
SET FactorValue = FactorValue * 1.1
WHERE FactorID = 1;
--  Check constraint condition
IF (SELECT FactorValue 
FROM Emission_Factor 
WHERE FactorID = 1) > 100
BEGIN
-- Rollback only to savepoint
ROLLBACK TRANSACTION SavePoint;
PRINT ' value exceeded limit';
END
ELSE
BEGIN
-- Log old and new values
INSERT INTO Emission_Factor_Log (FactorID, OldValue, NewValue)
SELECT 1, @oldValue, FactorValue
FROM Emission_Factor 
WHERE FactorID = 1;
END
COMMIT TRANSACTION;
...
SET emission_limit = emission_limit + 100 
WHERE target_id = 1;
COMMIT;
-- Run in window 2
BEGIN TRANSACTION;
UPDATE Emission_Target 
SET emission_limit = emission_limit + 50 
WHERE target_id = 1;
WAITFOR DELAY '00:00:05';
UPDATE Emission_Record 
SET quantity_used = quantity_used + 1 
WHERE record_id = 1;
COMMIT;
-- SQL Server detects deadlock and automatically rolls back one transaction 
--task4
BEGIN TRANSACTION;
-- First update
UPDATE Emission_Target
SET emission_limit = 22000
WHERE organization_id = 1 AND scope_id = 1;
-- Savepoint
SAVE TRANSACTION SavePoint;
-- Second update 
UPDATE Emission_Target
SET emission_limit = 5000
WHERE organization_id = 999;
-- If no rows affected  error condition
IF @@ROWCOUNT = 0
BEGIN
PRINT 'Invalid';
ROLLBACK TRANSACTION SavePoint;
...
BEGIN
INSERT INTO Site (organization_id, site_name, location)
VALUES (@org_id, @site_name, @location);
COMMIT TRANSACTION;
PRINT 'Site added ';
END
ELSE
...
(site_id, supplier_id, scope_id, source_id, record_date, quantity_used, emission_factor)
VALUES
(@site_id, @supplier_id, @scope_id, @source_id, @date, @quantity, @factor);
COMMIT TRANSACTION;
...
JOIN Site as  S ON ER.site_id = S.site_id
WHERE S.organization_id = @org_id
AND ER.scope_id = @scope_id
AND YEAR(ER.record_date) = YEAR(@date);
-- Get target limit
SELECT @limit = emission_limit
